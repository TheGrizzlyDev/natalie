...
describe "Array#pack with format 'M'" do
  ...
  fit "appends a soft line break at the end of an encoded string" do
    ["a"].pack("M").should == "a=\n"
  end

  fit "does not append a soft break if the string ends with a newline" do
    ["a\n"].pack("M").should == "a\n"
  end

  fit "encodes one element for each directive" do
    ["a", "b", "c"].pack("MM").should == "a=\nb=\n"
  end

  fit "encodes byte values 33..60 directly" do
    [ [["!\"\#$%&'()*+,-./"], "!\"\#$%&'()*+,-./=\n"],
  ...
  fit "encodes byte values 62..126 directly" do
    [ [[">?@"],                         ">?@=\n"],
  ...
  fit "encodes an '=' character in hex format" do
    ["="].pack("M").should == "=3D=\n"
  end

  fit "encodes an embedded space directly" do
    ["a b"].pack("M").should == "a b=\n"
  end

  fit "encodes a space at the end of the string directly" do
    ["a "].pack("M").should == "a =\n"
  end

  fit "encodes an embedded tab directly" do
    ["a\tb"].pack("M").should == "a\tb=\n"
  end

  fit "encodes a tab at the end of the string directly" do
    ["a\t"].pack("M").should == "a\t=\n"
  end

  fit "encodes an embedded newline directly" do
    ["a\nb"].pack("M").should == "a\nb=\n"
  end

  fit "encodes 0..31 except tab and newline in hex format" do
  ...
  fit "encodes a tab followed by a newline with an encoded newline" do
    ["\t\n"].pack("M").should == "\t=\n\n"
  end

  fit "encodes 127..255 in hex format" do
  ...
  fit "emits a soft line break when the output exceeds 72 characters when passed '*', 0, 1, or no count modifier" do
  ...
  fit "emits a soft line break when the output exceeds count characters" do
    [ [["abcdefghi"],        "M2", "abc=\ndef=\nghi=\n"],
      [["abcdefghi"],        "M3", "abcd=\nefgh=\ni=\n"],
      [["abcdefghi"],        "M4", "abcde=\nfghi=\n"],
      [["abcdefghi"],        "M5", "abcdef=\nghi=\n"],
